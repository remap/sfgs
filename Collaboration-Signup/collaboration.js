...
function register()
{
	...
	if(term==true)
	{
			...
			xhttp.onreadystatechange = function() 
			{
				if (xhttp.readyState == 4 && xhttp.status == 200) 
				{
					...
					if(response=='emailfailed')
					{
						...
						document.getElementById('success_msg').innerHTML='Form been submitted. Email sending failed';
						document.getElementById('error_msg').style.display='none';
						return false;
					}
					else
			...
			xhttp.open("GET", "../../custom-code/functions.php?method=new_register&wp_email="+wp_email+"&fn="+fn+"&ln="+ln+"&loc="+loc+"&aff="+aff+"&email1="+email1+"&email2="+email2+"&website="+website+"&pass1="+pass1+"&pass2="+pass2, true);
			xhttp.send();
	}
	else
	{
...
function update()
{
	...
	var wp_email=document.getElementById('wp_email').value;
			...
			xhttp.open("GET", "../../custom-code/functions.php?method=update&wp_email="+wp_email+"&fn="+fn+"&ln="+ln+"&loc="+loc+"&aff="+aff+"&email1="+email1+"&email2="+email2+"&website="+website+"&current_pass1="+current_pass1, true);
			xhttp.send();
