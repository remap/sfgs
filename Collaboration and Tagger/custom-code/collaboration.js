...
function register()
{
	...
	if(term==true)
	{
			...
			xhttp.open("GET", "../custom-code/functions.php?method=new_register&&fn="+fn+"&ln="+ln+"&loc="+loc+"&aff="+aff+"&email1="+email1+"&email2="+email2+"&website="+website+"&pass1="+pass1+"&pass2="+pass2+"&script_coverage="+script_coverage, true);
			xhttp.send();
	}
	else
	{
	...
	var pass=document.getElementById('pass').value;
			...
			xhttp.onreadystatechange = function() 
			{
				if (xhttp.readyState == 4 && xhttp.status == 200) 
				{
					...
					if(response=='success')
					{
						window.location='http://searchforglobalsong.com/author/tagger/';
						return false;
					}
					else
					{
			...
			xhttp.open("GET", "../custom-code/functions.php?method=collab_login&email="+email+"&pass="+pass, true);
			xhttp.send();
	...
	alert(email);
			...
			xhttp.onreadystatechange = function() 
			{
				if (xhttp.readyState == 4 && xhttp.status == 200) 
				{
					...
					{
						document.getElementById('error_msg').style.display='block';
						document.getElementById('error_msg').innerHTML='Email sent to '+email;
						document.getElementById('form_submit').style.display='block';
						document.getElementById('form_sending_recovery').style.display='none';
					...
					{
						document.getElementById('error_msg').style.display='block';
						document.getElementById('error_msg').innerHTML='Password reset but email sending failed';
						document.getElementById('form_submit').style.display='block';
						document.getElementById('form_sending_recovery').style.display='none';
						document.getElementById('email').value='';
						return false;
				...
				{
						document.getElementById('form_submit').style.display='none';
						document.getElementById('form_sending_recovery').style.display='block';
						document.getElementById('error_msg').style.display='none';
						return false;
				}
		 	}
			xhttp.open("GET", "../custom-code/functions.php?method=reset_pass&email="+email, true);
			xhttp.send();
	...
	var scene=document.getElementById('scene').value;
			...
			xhttp.onreadystatechange = function() 
			{
				if (xhttp.readyState == 4 && xhttp.status == 200) 
				{
					...
					{
						document.getElementById('reg_form').style.display='none';
						document.getElementById('success_msg').innerHTML='Thank you for contributing to Search for Global Song, you will see your video integrated on our YouTube Channel. Stay tuned!';
						document.getElementById('error_msg').style.display='none';
						return false;
					}
					if(response=='emailfailed')
...
function update()
{
	...
	var script_coverage=document.getElementById('script_coverage').value;
			...
			xhttp.open("GET", "../custom-code/functions.php?method=update&fn="+fn+"&ln="+ln+"&loc="+loc+"&aff="+aff+"&email1="+email1+"&email2="+email2+"&website="+website+"&current_pass1="+current_pass1+"&script_coverage="+script_coverage, true);
			xhttp.send();
...
function change_password()
{
	...
	var new_pass2=document.getElementById('new_pass2').value;
			...
			xhttp.open("GET", "../custom-code/functions.php?method=change_password&current_pass="+current_pass+"&new_pass1="+new_pass1+"&new_pass2="+new_pass2, true);
			xhttp.send();
